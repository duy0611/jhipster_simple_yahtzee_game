(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('simpleYahtzeeApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-simpleYahtzeeApp-error');
                var entityKey = httpResponse.headers('X-simpleYahtzeeApp-params');
                if (errorHeader) {
